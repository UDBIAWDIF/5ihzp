...
function getCurrentUserId() {
	return intval(session('adm_id'));
	//return intval(Session::get(APP_SESSION_FLAG . 'id'));
}
function getCurrentUserName() {
	return trim(session('adm_name'));
	//return intval(Session::get(APP_SESSION_FLAG . 'id'));
}
function getCurrentGroupId() {
	return intval(session('adm_gid'));
 ...
 *
 * 判断该天是否被预定
 * @param unknown_type $deliverylist
 * @param unknown_type $year
 * @param unknown_type $mon
 * @param unknown_type $add_mon
 * @param unknown_type $day
 */
function slotOrderStatus($deliverylist, $year, $mon, $add_mon, $day) {
	if ($mon + $add_mon > 12) {
		$year = $year + 1;
		$mon = $mon + $add_mon - 12;
	} else {
	...
	foreach ( $deliverylist as $k => $v ) {
		if ($v ['de_stime']) {
			if (strtotime ( $time ) >= strtotime ( $v ['de_stime'] ) && strtotime ( $time ) <= strtotime ( $v ['de_etime'] )) {
				return array ('de_id' => $v ['de_id'], 'de_status' => $v ['de_status'], 'de_order' => $v ['de_ord_id'], 'color' => $v ['color'], 'ord_admin_id'=>$v['ord_admin_id'],'ord_name'=>$v['ord_name']);
			}
		} else {
			if (strpos ( $v ['de_ord_time'], $time ) !== false) {
				return array ('de_id' => $v ['de_id'], 'de_status' => $v ['de_status'], 'de_order' => $v ['de_ord_id'], 'color' => $v ['color'], 'ord_admin_id'=>$v['ord_admin_id'],'ord_name'=>$v['ord_name']);
			}
		}
	}
...
function resizeImg($srcFile, $dstFile, $newWidth, $newHeight, $unlink = false) {
	...
	// 操作系统不同,中文文件名存储的的最终名字也不同,此方法提供获取正确文件名的方法
function getFilenameBySys($p_file) {

	return iconv('UTF-8', 'gb2312', trim($p_file));

/* 		if (strtoupper(substr(PHP_OS,0,3)) == 'WIN') {
		$file = iconv('UTF-8', 'gb2312', trim($p_file));
	} else {
		$file = trim($p_file);
	}
...
function verCodeFill0AndToNum($p_verCode) {
	//echo str_pad($input, 10, "-=", STR_PAD_LEFT);
	$verNumArray = explode('.', trim($p_verCode));
	$bound = count($verNumArray);

	if($bound < 4) {
		$verNumArray = array_merge($verNumArray, array_fill($bound, 4 - $bound, ''));
	}

	foreach($verNumArray as $k => $v) {
		$verNumArray[$k] = str_pad($v, 4, '0', STR_PAD_LEFT);
	}
...
function getPenname($name){

	$model = M('Users');

	if(empty($name)){
		$id = getCurrentUserId();
		$user = $model->field('penname')->find($id);
		$name = $user['penname'];
	}
