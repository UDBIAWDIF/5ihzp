...
//	private $upload_path;
	...
	public function getApkInfo($file) {
		...
		$apk_info = array(
					...
					'min_os_ver' => D('Firmware')->genOrderByLevel($ap->getAttribute('manifest/uses-sdk', 'android:minSdkVersion')),
		);
	...
	public function getAppinfoFromGooplay($app_url) {
		...
		import('@.ORG.phpQuery');
		phpQuery::newDocumentFile($app_url, null, C('PROXY_FOR_GOOGLE'));

		$icon = pq('.doc-banner-icon > img:eq(0)')->attr('src');
		if(!empty($icon)) {
			$icon = $this->_chgGooplayImgSize($icon);
			$return_data['icon91'] = $this->getRemoteImgs($icon, C('IMG_UP_TYPE.APP_ICON'), 'roundedCorner', true, C('PROXY_FOR_GOOGLE'));
		} else {
		...
		$return_data['softwareVersion'] = pq('dd[itemprop="softwareVersion"]')->html();
		return $return_data;
	...
	public function getIpaInfo($file) {
		...
		if(!empty($application)) {
			$ver_code = $application['CFBundleShortVersionString'] ? $application['CFBundleShortVersionString'] : $application['CFBundleVersion'];
			$apk_info = array(
	...
	public function roundedCorner($p_src_img, $p_dst_img, $ext_rename = true){
		...
		$rounder = ceil(min($img_size[0], $img_size[1]) * (87/512));
...
/* 					if($up_ftp_result) {
	...
	public function upload_img($img_type, $checksize = false, $up_to_ftp = false, $trigger = null) {
		...
		if (!self::$h_upload->upload()) {	//上传失败
			...
			$up_result['error'] = 0;
			$up_result['url'] = substr($file_full_path, strlen(BASE_PATH));
			...
			if(self::$h_upload->maxWidth && self::$h_upload->maxWidth < $img_size[0]){
				...
				/* if(!$check_result) {
					...
					$save_file = $this->_imgExtRename($file_full_path);
					$resize_file = $this->_resizeImg($save_file);
					if($resize_file) {
...
/* 				if($up_ftp_result) {
	 ...
	 */
	public function uploadFtpFile($p_file, $p_config, $singleFtp = false, $singleSuccess = true) {
		return uploadFtpFile($p_file, $p_config, $singleFtp, $singleSuccess);
	}
