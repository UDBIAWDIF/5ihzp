...
class Image {
	 ...
	 * @param boolean $interlace 启用等比缩放
	  +----------------------------------------------------------
	 * @return void
	  +----------------------------------------------------------
	 */
	static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true, $byScale=true) {
		// 获取原图信息
		...
		if ($info !== false) {
			...
			if ('gif' == $type || 'png' == $type) {
				$alpha_color = imagecolorallocatealpha($thumbImg,255,255,255,127);
				imagealphablending($thumbImg, false);
				imagesavealpha($thumbImg, true);
				imagefilledrectangle($thumbImg, 0, 0, $width, $height, $alpha_color);				
				imagefill($thumbImg, 0, 0, $alpha_color);
			}
...
// 				$background_color = imagecolorallocate($thumbImg, 0, 255, 0);  //  指派一个绿色
				...
				imagecolortransparent($thumbImg, $alpha_color);  //  设置为透明色，若注释掉该行则输出绿色的图
			}
			...
			// 生成图片
			$thumbExt = substr($thumbname, strrpos($thumbname, '.')+1);
			$type = ($thumbExt == 'jpg')? 'jpeg' : $type; 
