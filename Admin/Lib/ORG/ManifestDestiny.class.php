...
/* IPA文件信息解析工具 For example:

$application = ManifestDestiny::processIPA($result['picurl']);
if(!empty($application)) {
	$data['name'] 	 = $application['CFBundleDisplayName'];
	$data['version'] = $application['CFBundleVersion'];
	$data['key']     = $application['CFBundleIdentifier'];
	$data['os'] 	 = 1;
} else {
	$data['check'] = false;
	$data['msg']   = 'apk解析失败';
...
class ManifestDestiny {
	...
	public static function processIPA($ipaname) {
		...
		if ($zip->open($ipaname) === TRUE) {
			//Read the Info.plist file into memory
			$index = $zip->locateName('Info.plist', ZIPARCHIVE::FL_NODIR);
			$bplist = $zip->getFromIndex($index);
			$zip->close();
		} else {
	...
	* @功能：产生规律性文件名
	*/
	public static function getAttachName($p_suffix, $p_basedir=''){
		$time = time();
		$hash = md5($time.mt_rand(1000, 9999));
		$path = $p_basedir.($p_basedir ? '' : '/') . date('Y/m/d', $time) . '/' . substr($hash, 0, 2) . '/' . $hash . $p_suffix;
...
abstract class CFBinaryPropertyList {
  ...
  /**
   * a value (i.e. write the binary representation as big endian to a string) with the specified size
...
}//end class CFBinaryPropertyList
