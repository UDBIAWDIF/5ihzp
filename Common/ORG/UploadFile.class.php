	...
	private function save($file) {
		...
		if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
			...
			if(false !== $image) {
				...
				// 生成图像缩略图
				import($this->imageClassPath, BASE_PATH . 'Common');
				$realFilename	= $this->autoSub?basename($file['savename']):$file['savename'];
				// $fileExtension	= getFileSuffix($realFilename);
				$fileExtension	= strtolower($file['extension']);
				for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
					$thumbname	= $thumbPath
									.$thumbPrefix[$i]
									.substr($realFilename,0,strrpos($realFilename, '.'))
									.$thumbSuffix[$i]
									.'.'
									.$fileExtension;
	...
	public function upload($savePath ='') {
		...
		if(!is_dir($savePath)) {
			...
			if(is_dir(base64_decode($savePath))) {
				...
				// 尝试创建目录
				if(!mk_dir($savePath)){	//此处要改成可创建多级目录的方法 By UID 2012.08.13
